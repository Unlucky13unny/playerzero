...
export function VerificationSection({ profileUserId }: VerificationSectionProps) {
  ...
  const [targetUserIsPaid, setTargetUserIsPaid] = useState(false)
  const { user } = useAuth()
  const trialStatus = useTrialStatus()

  useEffect(() => {
    loadLastVerification()
    checkTargetUserPaymentStatus()
  }, [user, profileUserId])

  const checkTargetUserPaymentStatus = async () => {
    ...
    // If viewing another user's profile, fetch their profile to check payment status
    try {
      ...
      console.error('Error checking target user payment status:', err)
      setTargetUserIsPaid(false)
    }
  }
